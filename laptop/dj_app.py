...
DRUM_TRIGGER_CM = 30  # ultrasonic distance threshold (cm)
SAMPLE_RATE = 44100
CHANNELS = 2
...
def _sine_wave(
    freq: float, duration: float = 0.6, sample_rate: int = SAMPLE_RATE
) -> np.ndarray:
...
def _synth_drum(index: int, distance: float = 30.0) -> pygame.mixer.Sound:
    """Very rough drum synthesis so the app works without .wav files.
    Distance from the ultrasonic sensor modulates the pitch of the synth.
    (Closer = higher pitch/faster envelopes)
    """
    sr = SAMPLE_RATE
    # Constrain and invert distance to a multiplier (closer -> higher mult)
    # e.g. 5cm -> 2.0x pitch, 30cm -> 1.0x pitch
    d = max(5.0, min(30.0, float(distance)))
    pitch_mod = 1.0 + (30.0 - d) / 25.0

    if index == 0:  # kick -> fat sub-bass EDM kick
        t = np.linspace(0, 0.4, int(sr * 0.4), dtype=np.float32)
        freq = 150 * pitch_mod * np.exp(-15 * t)
        arr = np.sin(2 * np.pi * freq * t)
        arr = np.tanh(arr * 2)  # add overdrive
    elif index == 1:  # snare -> snappy EDM snare
        t = np.linspace(0, 0.25, int(sr * 0.25), dtype=np.float32)
        tone = 0.6 * np.sin(2 * np.pi * 200 * pitch_mod * t) * np.exp(-15 * t)
        noise = 0.5 * np.random.randn(len(t)).astype(np.float32) * np.exp(-12 * t)
        arr = tone + noise
    elif index == 2:  # hi-hat -> crisp EDM hat
        t = np.linspace(0, 0.1, int(sr * 0.1), dtype=np.float32)
        arr = np.random.randn(len(t)).astype(np.float32) * np.exp(-25 * pitch_mod * t)
    elif index in (3, 4):  # toms -> percussive synth pluck
        base = 120 if index == 3 else 80
        freq = base * pitch_mod
        t = np.linspace(0, 0.35, int(sr * 0.35), dtype=np.float32)
        arr = np.sin(2 * np.pi * freq * np.exp(-8 * t) * t)
    elif index == 5:  # clap -> layered noise bursts
        t = np.linspace(0, 0.25, int(sr * 0.25), dtype=np.float32)
        noise = np.random.randn(len(t)).astype(np.float32)
        env1 = np.exp(-40 * t)
        env2 = np.pad(np.exp(-40 * t[: -int(sr * 0.02)]), (int(sr * 0.02), 0))
        env3 = np.pad(np.exp(-40 * t[: -int(sr * 0.04)]), (int(sr * 0.04), 0))
        arr = noise * (env1 + env2 + env3 * 0.8)
    else:  # synth lead pluck -> modern PLVTINUM style lead sound
        # PLVTINUM uses dark, metallic FM/Square bass/lead plucks
        t = np.linspace(0, 0.5, int(sr * 0.5), dtype=np.float32)
        base_freq = 110.0 * pitch_mod

        # FM synthesis: Carrier frequency modulated by Modulator
        mod = np.sin(2 * np.pi * (base_freq * 2) * t)
        arr = np.sin(2 * np.pi * base_freq * t + 1.5 * mod * np.exp(-5 * t))

        # apply square-like wave folding for a harsher texture
        arr = np.tanh(arr * 3) * np.exp(-6 * t)
...
def init_audio(song_path: Optional[str], verbose: bool) -> None:
    ...
    for i, fname in enumerate(DRUM_SOUNDS):
        ...
        if os.path.isfile(path):
            ...
            loaded_drums[str(i + 1)] = _synth_drum(
                i, 30.0
            )  # pre-load a default distance synth
            if verbose:
                print(f"[audio] Synthesised EDM synth {fname}")
...
def play_drum(drum_id: str, distance_cm: float) -> None:
    ...
    if distance_cm > DRUM_TRIGGER_CM:
        print(
            f"[drum]  Drum {drum_id} skipped — {distance_cm:.1f} cm > {DRUM_TRIGGER_CM} cm"
        )
        return
    idx = int(drum_id) - 1
    # Dynamically synthesize the drum/synth sound so distance affects pitch!
    sound = _synth_drum(idx, distance_cm)
    ...
    print(f"[audio] 🥁 EDM Synth  {name}  @ {distance_cm:.1f} cm  vel={velocity:.2f}")
...
LED feedback to ESP32:
  LED,<0-127>            Sent back over serial when POT2 changes
    ...
    group.add_argument(
        "--mock-interactive", action="store_true", help="Type messages manually"
    )
    ...
    parser.add_argument(
        "--verbose", action="store_true", help="Show debug/warning output"
    )
