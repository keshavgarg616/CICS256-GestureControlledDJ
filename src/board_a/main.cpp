...
void loop()
{
  ...
  {
    // Breathes, Potentiometer controls breathing speed
    FastLED.setBrightness(savedBrightness);
    int breatheSpeedVal = pot2; // 10 to 120 beats per minute
    if (breatheSpeedVal - lastBreatheSpeedVal > 15 || breatheSpeedVal - lastBreatheSpeedVal < -20)
    {
      lastBreatheSpeedVal = breatheSpeedVal;
    }
    uint8_t bpm = map(lastBreatheSpeedVal, 0, 4095, 30, 1);
